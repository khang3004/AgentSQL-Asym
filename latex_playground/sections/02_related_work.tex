...
The transition from academic-scale datasets (Spider, WikiSQL) to industry-representative benchmarks has been catalysed by the \highlight{BIRD} benchmark \cite{li_can_2023}, which introduces 12,751 question--SQL pairs across 95 large-scale databases containing real-world dirty data. BIRD requires models to reason over complex multi-table joins, date format variations, and domain-specific value conventions. However, recent audits have revealed that up to 10\% of BIRD annotations contain semantic ambiguities or outright errors \cite{floratou2024broken}, underscoring the need for systems that remain resilient to imperfect evaluation signals.

\subsection{Schema Pruning and Context Construction}
Providing an LLM with a full database schema---potentially hundreds of tables---exceeds context windows and degrades reasoning quality. \highlight{CHESS} \cite{talaei_chess_2024} proposed a two-phase embedding pipeline:
\begin{enumerate}
    \item \textbf{Offline}: Index all table representations into a FAISS vector store \cite{johnson2019faiss} using sentence-transformer embeddings \cite{xiao2024cpackpackedresourcesgeneral}.
...
\end{enumerate}
Building on pruned schemas, \highlight{MCI-SQL} \cite{wang_mci-sql_2026} demonstrated that enriching prompts with \emph{multiple facets} of context information---column data types, cardinality ratios (1:1 PK vs.\ 1:N FK), MIN/MAX statistics, and sample values---significantly improves schema linking accuracy. AgentSQL integrates both strategies sequentially: CHESS prunes the schema from $|\mathcal{S}|$ to $k$ tables, then MCI-SQL enriches the pruned subset with column-level metadata.

\subsection{Multi-Agent and Self-Correction Paradigms}
Multi-agent frameworks decompose the Text-to-SQL task into specialised sub-tasks. \highlight{MAC-SQL} \cite{wang_mac-sql_2025} employs a Decomposer, a Selector, and a Refiner operating collaboratively. \highlight{MAGIC} \cite{askari_magic_2024} generates self-correction guidelines from in-context exemplars, providing the LLM with an explicit error-checklist during generation. Execution-feedback approaches---\highlight{ReViSQL} \cite{zhu2026revisqlachievinghumanleveltexttosql} and \highlight{AGENTIQL} \cite{heidari_agentiql_2025}---iterate between generation and database execution to fix queries empirically.
